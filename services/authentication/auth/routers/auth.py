...
from auth.queries import create_user, get_user_by_email, verify_password, create_access_token, create_refresh_token, \
    get_current_user
from auth.schemas import UserOutSchema, UserCreateSchema, UserLoginSchema
...
@router.post('/login')
async def login(credentials: UserLoginSchema, response: Response, session: AsyncSession = Depends(get_session)):
    ...
    response.set_cookie('access_token', access_token, httponly=True, secure=True, samesite='lax', max_age=7200)
    return {
